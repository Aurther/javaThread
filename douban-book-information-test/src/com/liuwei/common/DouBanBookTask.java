...
import com.liuwei.pojo.DouBanBookInfo;
...
public class DouBanBookTask extends Thread {
	...
	public void run() {
		try {
			...
			for (int i = 0; i < 101; i++) {
				int start = i * Integer.valueOf(DouBanBookInfo.Constants);
				if (start > 0) {
					url_ = url + DouBanBookInfo.Constants + start;
				} else {
					url_ = url;
				}
				...
				while (dlIter.hasNext()) {
					try {
						...
						if (DouBanBookTask.isNotBlank(pointString)) {
							Double point = Double.valueOf(pointString);
							System.out.println(title + "\t" + point);
							if (point > 9)
								writeToFile("E:/doubanbook", title, point);
						}
					} catch (Exception e) {
						continue;
					}
	...
	private static boolean isNotBlank(String pointString) {
		return false;
	}
